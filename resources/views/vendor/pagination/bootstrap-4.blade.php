@if ($paginator->hasPages())
    <nav>
        <ul class="pagination">
                ...
                @if (is_array($element))
                    @foreach ($element as $page => $url)
                        @if ($page == $paginator->currentPage())
                            <li class="" aria-current="page"><span class="page-link text-dark p-active">{{ $page }}</span></li>
                        @else
                            <li class=""><a class="page-link text-dark" href="{{ $url }}">{{ $page }}</a></li>
                        @endif
                    @endforeach
                @endif
            @endforeach
            ...
            @endif
            {{-- <li>
                <p class="small text-muted" style="padding-top: 7px;padding-left: 5px;">
                    {!! __('Showing') !!}
